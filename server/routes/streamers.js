...
router.post('/', async (req, res) => {
    ...
    if (!name || !platform || initialCount === undefined) {
        return res
            .status(400)
            .json({ error: 'Missing required fields: name, platform, initialCount' });
    }
    ...
    try {
        ...
        const uuid_streamer = crypto.randomUUID();
        // Insert streamer
        const streamerResult = await client.query(
            'INSERT INTO streamers (id, name, platform) VALUES ($1, $2, $3) RETURNING id, name, platform',
            [uuid_streamer, name, platform]
        );

        const streamer = streamerResult.rows[0];
        const timestamp = Date.now();
        const uuid_subscriber_history = crypto.randomUUID();
        // Insert initial subscriber count
        await client.query(
            'INSERT INTO subscriber_history (id, streamer_id, count, timestamp) VALUES ($1, $2, $3, $4)',
            [uuid_subscriber_history, streamer.id, initialCount, timestamp]
        );
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query('DELETE FROM streamers WHERE id = $1 RETURNING id', [id]);
...
router.post('/:id/subscribers', async (req, res) => {
    ...
    try {
        ...
        const streamerResult = await pool.query('SELECT id FROM streamers WHERE id = $1', [streamerId]);
        ...
        const result = await pool.query(
            `
  INSERT INTO subscriber_history (id, streamer_id, count, timestamp)
  VALUES ($1, $2, $3, $4)
   RETURNING id, streamer_id, count, timestamp
  `,
            [uuid, streamerId, count, timestamp]
        );
