...
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    calculateStats(): void {
        ...
        const topStreamer = this.streamers.reduce((top, s) => {
            if (!top) return s;
            const latestCount =
                s.history && s.history.length > 0 ? s.history[s.history.length - 1].count : 0;
            const topCount =
                top.history && top.history.length > 0
                    ? top.history[top.history.length - 1].count
                    : 0;
    ...
    prepareCharts(): void {
        ...
        const sortedStreamers = [...this.streamers]
            .sort((a, b) => {
                const countA =
                    a.history && a.history.length > 0 ? a.history[a.history.length - 1].count : 0;
                const countB =
                    b.history && b.history.length > 0 ? b.history[b.history.length - 1].count : 0;
        ...
        this.comparisonData = {
            ...
            datasets: [
                {
                    label: 'Subscribers',
                    data: sortedStreamers.map((s) =>
                        s.history && s.history.length > 0
                            ? s.history[s.history.length - 1].count
                            : 0
                    ),
                    backgroundColor: sortedStreamers.map(
                        (_, i) => ['#FF6B9D', '#C44DFF', '#00B4FF', '#00D9A3', '#FFA800'][i % 5]
                    ),
                },
            ],
        };
